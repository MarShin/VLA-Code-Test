...
class HumanoidPlaceAppleInBowl(HumanoidPickPlaceEnv):
    ...
    def _load_scene(self, options: Dict):
        ...
        self.apple2 = builder.build(name="apple2")
        self.apple3 = builder.build(name="apple3")

    def evaluate(self):
        # TODO: would have to calibrate the threshold - also maybe sum of app dist <= threshold
        is_obj_placed = (
            (torch.linalg.norm(self.bowl.pose.p - self.apple.pose.p, axis=1) <= 0.05)
            and (torch.linalg.norm(self.bowl.pose.p - self.apple2.pose.p, axis=1) <= 0.05)
            and (torch.linalg.norm(self.bowl.pose.p - self.apple3.pose.p, axis=1) <= 0.05)
        )
        hand_outside_bowl = (
        ...
        # somehow useful
        is_grasped = (
            self.agent.right_hand_is_grasping(self.apple, max_angle=110)
            or self.agent.right_hand_is_grasping(self.apple2, max_angle=110)
            or self.agent.right_hand_is_grasping(self.apple3, max_angle=110)
        )
        return {
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                ...
                obj1_pose=self.apple.pose.raw_pose,
                obj2_pose=self.apple2.pose.raw_pose,
                obj3_pose=self.apple3.pose.raw_pose,
                # tcp_to_obj_pos=self.apple.pose.p - self.agent.right_tcp.pose.p,
                obj1_to_goal_pos=self.bowl.pose.p - self.apple.pose.p,
                obj2_to_goal_pos=self.bowl.pose.p - self.apple2.pose.p,
                obj3_to_goal_pos=self.bowl.pose.p - self.apple3.pose.p,
            )
        return obs
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        # NOTE: formulation becomes different when there are more than 1 target objec to to manipulate
        # should we do average dist?
        tcp_to_obj_dist = (
            torch.linalg.norm(self.apple.pose.p - self.agent.right_tcp.pose.p, axis=1)
            + torch.linalg.norm(self.apple2.pose.p - self.agent.right_tcp.pose.p, axis=1)
            + torch.linalg.norm(self.apple3.pose.p - self.agent.right_tcp.pose.p, axis=1)
        )
        reaching_reward = 1 - torch.tanh(5 * tcp_to_obj_dist)
        reward = reaching_reward  # max 1

        is_grasped = info["is_grasped"]
        reward += is_grasped  # max 1

        # encourage to bring apple to above the bowl then drop it.
        # NOTE: whether summing it up or use max() of 3 distance
        obj_to_goal_dist = (
            torch.linalg.norm(
        ...
        # TODO: adjust the threshold value
    ...
    def compute_normalized_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        return (
            self.compute_dense_reward(obs=obs, action=action, info=info) / 14
        )  # 8 + max 2 * 3 apples


@register_env("UnitreeG1PlaceAppleInBowlEnhanced-v1", max_episode_steps=100)
class UnitreeG1PlaceAppleInBowlEnhancedEnv(HumanoidPlaceAppleInBowl):
    """
    **Task Description:**
    ...
    **Goal Specification:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: Dict):
        ...
        with torch.device(self.device):
            ...
            # initialize the apple to be within reach
            xyz = torch.zeros((b, 3))
            xyz[:, :2] = randomization.uniform(low=0.025, high=0.05, size=(b, 2))
            qs = randomization.random_quaternions(b, lock_x=True, lock_y=True)
            xyz[:, 2] = 0.7335
            self.apple2.set_pose(Pose.create_from_pq(xyz, qs))

            # initialize the apple to be within reach
            xyz = torch.zeros((b, 3))
            xyz[:, :2] = randomization.uniform(low=-0.05, high=0.0, size=(b, 2))
            qs = randomization.random_quaternions(b, lock_x=True, lock_y=True)
            xyz[:, 2] = 0.7335
            self.apple3.set_pose(Pose.create_from_pq(xyz, qs))
